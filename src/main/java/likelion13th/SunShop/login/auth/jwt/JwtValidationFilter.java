...
/* 요청마다 JWT의 유효성을 검증하고,
 검증이 성공하면 SecurityContext에 인증(Authentication)을 주입하여
 이후 컨트롤러/서비스 계층에서 인증된 사용자 정보를 활용할 수 있도록 함.
 이미 인증된 요청은 필터를 다시 거치지 않고 바로 체인을 이어가도록 처리함.
 즉, 인증 흐름의 관문 역할을 하며 잘못된 토큰은 표준화된 에러 응답(JSON, 401)을 반환함.
*/